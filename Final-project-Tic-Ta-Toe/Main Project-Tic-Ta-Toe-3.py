...
myFont1 = 'arial', 14, 'bold'  # متغير لحفظ اعدادات الخط
bgcolor = '#70a1ff'  # متغير للون


# دالة إستدعاء الفريم الرئيسي fr1
...
# إعدادات الفريمات الرئيسية والفرعيه
...
# للانتقال من الصفحة الرئيسية لواجهة اللعبه
for frame in (fr1, FormFrame):
    frame.grid(row=1, column=0, sticky='news')



##############
# داله للتأكد من إدخال اسماء اللاعبين  وحفظها قبل الدخول للعبه
def Playersname():
...
# هذا الجزء هو خاص باعدادات  الواجهة الرئيسيه حيث يدخل كل لاعب الاسم الخاص به قبل بدء اللعب
var_1 = StringVar()
var_2 = StringVar()

label_1 = Label(fr1, text="Enter PlayerX Name", bd=1,
                relief=RIDGE, pady=2, padx=2, fon=myFont)
label_2 = Label(fr1, text="Enter PlayerO Name", bd=1,
                relief=RIDGE, pady=2, padx=2, fon=myFont)
entry_1 = Entry(fr1, bd=1, relief=RIDGE, width=20, fon=myFont)
entry_2 = Entry(fr1, bd=1, relief=RIDGE, width=20, fon=myFont)
button_1 = Button(fr1, text="Start Play VS Human", bd=1,
                  relief=RIDGE, pady=2, padx=2, fon=myFont, command=Playersname)
entry_1.focus()  # (entry_1) لوضع المؤشر داخل مربع النص
...
panel = Label(fr1, image=img, width=300)
panel.grid(row=3, column=1)


# زر الخروج من اللعبه
bclose = Button(saudidevorg, text="Exit", font=myFont,
                command=saudidevorg.destroy)
bclose.grid(row=4, column=0, pady=10)
...
def result():
    if (b1["text"] == "X" and b2["text"] == "X" and b3["text"] == "X"):
        b1.configure(background="#eccc68")  # تغيير لون الزر عند تطابق الشكل
        b2.configure(background="#eccc68")  # تغيير لون الزر عند تطابق الشكل
        b3.configure(background="#eccc68")  # تغيير لون الزر عند تطابق الشكل
...
# لاظهار الفورم الرئيسي قبل بدء اللعبة
raise_frame(fr1)
